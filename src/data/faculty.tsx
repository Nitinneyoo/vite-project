...
export const FacultyMember = () => {
    const featuredFaculty = teacherInfo.filter((teacherInfo: Faculty) => teacherInfo.isFeatured);
                            ...
                            <div key={teacherInfo.id} className="flex justify-center">
                                <BackgroundGradient className="flex flex-col rounded-[22px] bg-black dark:bg-zinc-900 overflow-hidden h-full max-w-sm">
                                    <div className="p-4 sm:p-6 flex flex-col items-center text-center flex-grow">
                                        <h2 className="text-3xl sm:text-3xl font-bold text-white dark:text-white">{teacherInfo.name}</h2>
                                        <p className="text-lg sm:text-xl text-white mt-4 mb-2 dark:text-neutral-200"> {teacherInfo.department}</p>
                                        <p className="text-sm sm:text-xl text-neutral-600 dark:text-neutral-400"> {teacherInfo.description}</p>
                                        <p className="text-sm sm:text-xl text-neutral-600 dark:text-neutral-400"> {teacherInfo.slug}</p>
                                    </div>
                                </BackgroundGradient>
